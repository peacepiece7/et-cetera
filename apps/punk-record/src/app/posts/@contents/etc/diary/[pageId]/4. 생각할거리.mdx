...
머지되면 바로 배포되기 떄문에 PR에서 feedback과 CI/CD QA가 이루어짐
...
요약하면 PR을 머지할 떄 release를 붙이면 trunk인데 github flow는 feature에서 pr, release 태그가 붙어서 테스트 되는 반면 trunk는 trunk에 머지하는 시점에 release 태그가 붙는 것으로 보인다.
