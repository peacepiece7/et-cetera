export const METADATA = {
  title: "blog | 서울에 위치한 시당 목록 출력하기",
  description: "서울에 위치한 시당 목록 출력하기",
  date: "",
  tags: [],
...
서울에 위치한 식당 목록을 출력하는 문제\
출력에 집계함수와 식당 정보를 출력해야한다.\
일반 정보(?)와 평균을 같이 출력하려면 group by가 들어가거나 서브쿼리로 평균을 구해야한다.\
group by 전에 where을 걸어서 조회 범위를 줄여주는 방식으로 풀었다.\
인라인 뷰로 평균을 구하는 방식도 있었다.
...
00035	삼촌식당	일식	80	서울특별시 강서구 가로공원로76가길	4.
```

결국 못맞춰서 gpt 한테 물어봤는데, group by로 묶어서 평균을 구해야하는 부분이 틀렸었다.
...
-- 조회1. 식당 id, 식당 이름, 음식 종류, 즐겨찾기수, 주소, 리뷰 평균(소수점 세자리 반올림)
-- 조건1. 서울에 위치한 식당
-- 정렬1. 평균 점수 desc
-- 정렬2. 즐겨찾기수 desc
...
WHERE ri.address LIKE '서울%'  -- 서울에 위치한 식당만 조회
GROUP BY ri.rest_id, ri.rest_name, ri.food_type, ri.favorites, ri.address
ORDER BY score DESC, ri.favorites DESC;  -- 리뷰 평균 점수와 즐겨찾기 수 기준 내림차순 정렬
```

다른 사람 정답\
제일 처음 내가 원했던 건데 from 절에서 서브쿼리로 평균을 구하고 있음

```sql
select ri.rest_id, ri.rest_name, ri.food_type, ri.favorites, ri.address, rr.score
from rest_info as ri join
    (
