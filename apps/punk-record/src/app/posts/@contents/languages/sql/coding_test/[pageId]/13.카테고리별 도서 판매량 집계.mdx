export const METADATA = {
  title: "blog | 카테고리별 도서 판매량 집계",
  description: "카테고리별 도서 판매량 집계",
  date: "",
  tags: [],
...
group by를 사용하는 간단한 문제\
이렇게 도서 판매량을 집계했는데\
`sum(bs.sales) over(partition by b.category range unbounded preceding)`\
group by에서 이미 `group by(b.category)`로 묶어 주고 있기 때문에\
category 분류가 되지 않는 문제를 겪었다. `sum(bs.sales)` + `group by(b.category)`만 작성하면 되는 문제였다.
