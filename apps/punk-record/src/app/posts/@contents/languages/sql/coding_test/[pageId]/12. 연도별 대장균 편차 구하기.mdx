export const METADATA = {
  title: "blog | 연도별 대장균 편차 구하기",
  description: "연도별 대장균 편차 구하기",
  date: "",
  tags: [],
...
group by로 연도별 가장 큰 대장균의 크기를 구하고\
각 대장균의 연도별 편차를 구하면된다.\
이 문제 풀때 주의점이 **date_format을 사용하면 string = int 비교가 되어서 실패**한다.\
YEAR, MONTH, DAY를 사용해서 그룹핑 해야한다.
...
select
    A.year,
    (A.max_colony_size_by_year - ed.size_of_colony) as year_dev,
    ed.id
from ecoli_data ed
join (
    select
     YEAR(differentiation_date) as year,
     max(size_of_colony) as max_colony_size_by_year
    from ecoli_data
...
gpt 풀이\
윈도우 함수를 사용하니까 훨씬 간단하고, 테이블 스캔을 한 번만 하니까 성능도 더 좋다.
```sql
SELECT
    YEAR(differentiation_date) AS year,
